...
def main():
    ...
    for stock in STOCKS:
        ...
        print(f"{'=' * 70}")
        print(
            f"{'配置':<30} {'总收益':>8} {'夏普':>7} {'回撤':>8} {'交易':>5} {'胜率':>7} {'盈亏比':>7}"
        )
        print("-" * 70)
        ...
        for cfg in CONFIGS:
            ...
            if result and result.get("success"):
                ...
                print(
                    f"{cfg['label']:<30} {tr:>+7.2f}% {sr:>6.2f} "
    ...
    for name, res in results.items():
        ...
        if old:
            print(f"\n{name}:")
            print(
                f"  旧退出: {old['total_return']:+.2f}% | 盈亏比 {old['profit_factor']:.2f} | 夏普 {old['sharpe']:.2f}"
            )
            print(f"  最优新: {best['label']}")
            print(
                f"  新退出: {best['total_return']:+.2f}% | 盈亏比 {best['profit_factor']:.2f} | 夏普 {best['sharpe']:.2f}"
            )
            print(
                f"  改善 : {best['total_return'] - old['total_return']:+.2f}pp (盈亏比 {old['profit_factor']:.2f}→{best['profit_factor']:.2f})"
            )
    ...
    print(
        "   退出优先级: 固定止盈 > 最高点回撤止损 > ATR硬止损 > 趋势反转 > 最大持有天数 > RSI动量衰竭"
    )
