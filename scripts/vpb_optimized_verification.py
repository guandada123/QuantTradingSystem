...
def main():
    ...
    for stock in STOCKS:
        ...
        print(f"{'=' * 85}")
        print(
            f"{'配置':<35} {'收益':>8} {'年化':>8} {'夏普':>7} {'回撤':>8} {'交易':>5} {'胜率':>7} {'盈亏比':>7}"
        )
        print("-" * 85)
        ...
        for cfg in CONFIGS:
            ...
            if result and result.get("success"):
                ...
                print(
                    f"{mark} {label:<33} {total_ret:+7.2f}% {annual_ret:+7.2f}% "
    ...
    for entry in all_rows:
        ...
        old = next(
            (c for c in configs if "旧退出" in c.get("label", "") and "error" not in c), None
        )
