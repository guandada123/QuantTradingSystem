...
def make_mock_db(account_data=None, position_data=None, order_data=None):
    ...
    default_account = MockRow(
        {"available_cash": 1000000.0, "total_assets": 1200000.0, "market_value": 200000.0}
    )
...
class TestOrderCreation:
    ...
    def test_create_order_basic(self):
        ...
        order = mgr.create_order(
            ts_code="600519.SH", direction="BUY", order_type="LIMIT", price=1800.0, quantity=100
        )
    ...
    def test_create_order_sell(self):
        ...
        mgr = OrderManager(db=db)
        order = mgr.create_order(
            ts_code="000001.SZ", direction="SELL", order_type="LIMIT", price=15.5, quantity=200
        )
    ...
    def test_create_order_with_strategy(self):
        ...
        mgr = OrderManager(db=db)
        order = mgr.create_order(
            ts_code="600036.SH", direction="BUY", price=40.0, quantity=500, strategy_name="MA_CROSS"
        )
    ...
    def test_create_order_market_type(self):
        ...
        mgr = OrderManager(db=db)
        order = mgr.create_order(
            ts_code="601318.SH", direction="BUY", order_type="MARKET", price=50.0, quantity=100
        )
    ...
    def test_create_order_invalid_direction(self):
        ...
        with pytest.raises(ValueError):
            mgr.create_order(ts_code="600519.SH", direction="INVALID", price=1800.0, quantity=100)
...
class TestStopOrders:
    ...
    def test_stop_order_missing_trigger_price(self):
        ...
        with pytest.raises(ValueError, match="trigger_price"):
            mgr.create_order(
                ts_code="600519.SH", direction="BUY", order_type="STOP", price=1300.0, quantity=100
            )
...
class TestOrderExecution:
    ...
    def test_execute_buy_insufficient_funds(self):
        ...
        account_data = {"available_cash": 10000.0, "total_assets": 50000.0, "market_value": 40000.0}
    ...
    def test_execute_sell_insufficient_position(self):
        ...
        position_data = {"total_quantity": 100, "available_quantity": 100, "cost_price": 1800.0}
...
class TestPositionManager:
    ...
    def test_open_position_new(self):
        ...
        result = mgr.open_position(
            ts_code="600519.SH", quantity=100, price=1800.0, direction="LONG"
        )
        assert result["success"] is True
    ...
    def test_open_position_insufficient_funds(self):
        """测试资金不足开仓失败"""
        account_data = {"available_cash": 100.0, "total_assets": 100.0, "market_value": 0.0}
        db = make_mock_db(account_data=account_data)
        mgr = PositionManager(db=db)
        result = mgr.open_position(ts_code="600519.SH", quantity=100, price=1800.0)
        assert result["success"] is False
        assert "资金不足" in result["error"]

    def test_close_position_success(self):
        """测试平仓成功"""
        position_data = {"total_quantity": 200, "available_quantity": 200, "cost_price": 1800.0}
        db = make_mock_db(position_data=position_data)
        mgr = PositionManager(db=db)
        result = mgr.close_position(ts_code="600519.SH", quantity=100, price=1900.0)
        assert result["success"] is True
    ...
    def test_close_position_no_position(self):
        ...
        mgr = PositionManager(db=db)
        result = mgr.close_position(ts_code="999999.SH", quantity=100, price=10.0)
        assert result["success"] is False
        assert "未找到" in result["error"]

    def test_close_position_insufficient(self):
        """测试平仓数量超过可用持仓"""
        position_data = {"total_quantity": 100, "available_quantity": 50, "cost_price": 20.0}
        db = make_mock_db(position_data=position_data)
        mgr = PositionManager(db=db)
        result = mgr.close_position(ts_code="000001.SZ", quantity=100, price=25.0)
        assert result["success"] is False
        assert "不足" in result["error"]

    def test_pnl_calculation_profit(self):
        """测试盈利计算"""
        position_data = {"total_quantity": 100, "available_quantity": 100, "cost_price": 50.0}
        db = make_mock_db(position_data=position_data)
        mgr = PositionManager(db=db)
...
class TestRiskController:
    ...
    def test_risk_check_pass(self):
        """测试正常风控通过"""
        controller = RiskController(db=None, max_position_ratio=0.30, max_total_positions=3)
        result = controller.check_trade_risk(
            ts_code="600519.SH",
            action="BUY",
            quantity=100,
            account_info={"total_assets": 1000000, "positions": {}, "total_positions": 0},
        )
    ...
    def test_risk_check_position_limit(self):
        """测试仓位比例超标"""
        controller = RiskController(db=None, max_position_ratio=0.30, max_total_positions=5)
        result = controller.check_trade_risk(
            ts_code="600519.SH",
            action="BUY",
            quantity=100,
