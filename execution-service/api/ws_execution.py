...
async def broadcast_order_update(
    order_id: str, ts_code: str, direction: str, status: str, price: float, quantity: int
) -> int:
    """广播订单状态变更"""
...
async def broadcast_risk_alert(ts_code: str, risk_type: str, detail: str) -> int:
    """广播风控触发"""
    return await ws_manager.broadcast(
        WSType.RISK_ALERT, {"ts_code": ts_code, "risk_type": risk_type, "detail": detail}
    )


async def broadcast_position_update(ts_code: str, action: str, quantity: int, price: float) -> int:
    """广播持仓变更"""
    return await ws_manager.broadcast(
