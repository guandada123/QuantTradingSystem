...
risk_events_total = Counter('risk_events_total', 'Total number of risk events', ['event_type', 'level'])
circuit_breaker_open = Gauge('circuit_breaker_open', 'Circuit breaker status (1=open/blocked, 0=closed/allowed)')
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(f"熔断器状态: {'OPEN' if cb_status['is_open'] else 'CLOSED'} (连续止损 {cb_status['consecutive_losses']} 次)")
    except Exception as e:
    ...
    if alert_service:
        try:
            await alert_service.send_system_error(
                "execution-service", "服务已启动"
            )
        except Exception:
            pass  # 启动通知失败不影响服务
...
exec_ws_manager._on_count_change = lambda n: websocket_connections_active.labels(service="execution").set(n)
