...
class CircuitBreaker:
    ...
    def record_loss(self):
        ...
        if self._consecutive_losses >= self.max_consecutive_losses:
            ...
            logger.warning(
                f"熔断触发！连续{self._consecutive_losses}次止损，暂停交易{self.cooldown_minutes}分钟"
            )
            ...
            "cooldown_remaining_minutes": max(
                ...
                round(
                    (
                        ...
                        - (datetime.now() - self._opened_at).total_seconds() / 60
                    ),
                    1,
                ),
            )
            if self._opened_at and self._is_open
            else 0,
        }
...
class RiskController:
    ...
    def pre_trade_check(
        self, ts_code: str, direction: str, quantity: int, price: float
    ) -> dict[str, Any]:
        """
        ...
        if direction == "BUY":
            ...
            if is_new_position and position_count >= self.max_total_positions:
                risks.append(
                    f"持仓数量超标：当前{position_count}只 >= 上限{self.max_total_positions}只"
                )
            ...
            pos = (
                self.db.execute(
                    text("SELECT available_quantity FROM positions WHERE ts_code = :tc"),
                    {"tc": ts_code},
                )
                .mappings()
                .fetchone()
        ...
        day_pnl = (
            self.db.execute(
                text("SELECT day_profit_loss FROM accounts WHERE account_id = :aid"),
                {"aid": self.account_id},
            )
            .mappings()
            .fetchone()
        ...
        if day_pnl and day_pnl["day_profit_loss"] is not None:
            day_loss_ratio = (
                abs(float(day_pnl["day_profit_loss"])) / total_assets if total_assets > 0 else 0
            )
            if float(day_pnl["day_profit_loss"]) < 0 and day_loss_ratio > self.max_daily_loss:
                risks.append(
                    f"当日亏损超标：{day_loss_ratio * 100:.1f}% > {self.max_daily_loss * 100}%"
                )
    ...
    def check_trade_risk(
        self, ts_code: str, action: str, quantity: int, account_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if action == "BUY":
            ...
            current_ratio = (
                current_position.get("market_value", 0) / total_assets if total_assets > 0 else 0
            )

            if current_ratio > self.max_position_ratio:
                risks.append(
                    f"仓位超标：{current_ratio * 100:.1f}% > {self.max_position_ratio * 100}%"
                )

        if action == "BUY":
            total_positions = account_info.get("total_positions", 0)
            if total_positions >= self.max_total_positions and ts_code not in account_info.get(
                "positions", {}
            ):
    ...
    def check_stop_loss(
        self, ts_code: str, cost_price: float, current_price: float
    ) -> dict[str, Any]:
        """检查是否需要止损"""
                ...
                if settings.AUTO_EXECUTE_STOP_LOSS and available_quantity > 0:
                    try:
                        ...
                        close_result = pm.close_position(
                            ts_code=ts_code, quantity=available_quantity, price=current_price
                        )
                        if close_result["success"]:
                            ...
                            logger.info(
                                f"止损自动平仓: {ts_code} {available_quantity}股 @{current_price}"
                            )
                        else:
                ...
                if settings.AUTO_EXECUTE_TAKE_PROFIT and available_quantity > 0:
                    try:
                        ...
                        if close_result["success"]:
                            ...
                            logger.info(
                                f"止盈自动平仓: {ts_code} {available_quantity}股 @{current_price}"
                            )
                    except Exception as e:
