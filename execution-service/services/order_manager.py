...
class OrderManager:
    ...
    def __init__(self, db: Session, commission_rate: float = 0.0003, tax_rate: float = 0.001,
    ...
    def create_order(
        ...
        """创建订单并持久化到DB — 含输入验证"""
        # 验证输入
        validation_error = self._validate_order(ts_code, direction, order_type, price, quantity, trigger_price)
        if validation_error:
    ...
    def execute_order(self, order_id: str) -> Dict[str, Any]:
        ...
        ), {'oid': order_id}).mappings().fetchone()
        ...
        if direction == 'BUY':
            ...
            if available_cash < total_deduct:
                ...
                try:
                    ...
                    _fire_alert(alert_svc.send_order_rejected(
                        {"ts_code": ts_code, "direction": direction,
                         "quantity": quantity, "price": price, "order_id": order_id},
                ...
                try:
                    ...
                    _fire_alert(broadcast_order_update(
                        order_id, ts_code, direction, "REJECTED", price, quantity,
                    ))
                except Exception as e:
                    logger.debug(f"WS广播失败: {e}")
                return {'success': False, 'error': f'资金不足: 需要{total_deduct:.2f}, 可用{available_cash:.2f}'}
            ...
            existing = self.db.execute(text(
                "SELECT total_quantity, cost_price FROM positions WHERE ts_code = :tc"
        ...
        """), {
            ...
            'td': now.date(), 'tt': now.time(), 'created': now
        })
        ...
        try:
            from services.feishu_alert import get_alert_service
            alert_svc = get_alert_service()
            _fire_alert(alert_svc.send_order_filled({
                "order_id": order_id,
                "ts_code": ts_code,
                "direction": direction,
                "price": price,
        ...
        try:
            ...
            _fire_alert(broadcast_order_update(
                order_id, ts_code, direction, "FILLED", price, quantity,
            ))
    ...
    def list_orders(self, status: Optional[str] = None, limit: int = 20) -> List[Dict[str, Any]]:
        """从DB查询订单列表"""
        if status:
            ...
            """), {'status': status, 'limit': limit}).mappings().fetchall()
        else:
            rows = self.db.execute(text("""
                SELECT order_id, ts_code, direction, order_type, price, quantity, amount,
                       status, filled_price, filled_quantity, commission, tax,
    ...
    def _update_order_status(self, order_id: str, status: str, error_message: Optional[str] = None):
        """内部方法：更新订单状态"""
    ...
    def _validate_order(self, ts_code: str, direction: str, order_type: str,
                        price: Optional[float], quantity: int,
        ...
        if order_type == 'STOP':
            if trigger_price is None or trigger_price <= 0:
                return f"STOP单必须提供trigger_price（触发价格）"
    ...
    def _check_trading_status(self) -> Optional[str]:
    ...
    def check_stop_orders(self, price_map: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            if direction == 'BUY' and current_price >= trigger_price:
                should_trigger = True
            elif direction == 'SELL' and current_price <= trigger_price:
                should_trigger = True

            if should_trigger:
                order_id = row['order_id']
                logger.info(f"STOP订单触发: {order_id} {ts_code} {direction} @触发价={trigger_price} 现价={current_price}")
    ...
    def get_daily_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'positions': {
                ...
                'total_market_value': float(positions['total_market_value'] or 0) if positions else 0,
                'total_unrealized_pnl': float(positions['total_unrealized_pnl'] or 0) if positions else 0
            },
