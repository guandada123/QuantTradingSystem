...
class PositionManager:
    ...
    def __init__(self, db: Session, account_id: str = DEFAULT_ACCOUNT_ID,
    ...
    def open_position(
        ...
        ), {'aid': self.account_id, 'tc': ts_code}).mappings().fetchone()
        ...
        if existing:
            ...
            """), {
                ...
                'price': price, 'mv': new_mv, 'pnl': pnl, 'pnl_ratio': pnl_ratio,
                'strategy': strategy_name, 'aid': self.account_id, 'tc': ts_code
            })
        else:
            self.db.execute(text("""
            ...
            """), {
                'aid': self.account_id, 'tc': ts_code, 'dir': direction,
                'qty': quantity, 'cost': price, 'price': price, 'mv': trade_amount,
    ...
    def close_position(
        ...
        if available_qty < quantity:
            return {'success': False, 'error': f'可用持仓不足: 需要{quantity}股, 可用{available_qty}股'}
        ...
        # 更新账户现金
        account = self.db.execute(text(
            "SELECT available_cash, market_value FROM accounts WHERE account_id = :aid"
        ), {'aid': self.account_id}).mappings().fetchone()
        ...
        if new_qty == 0:
            self.db.execute(text(
                "DELETE FROM positions WHERE account_id = :aid AND ts_code = :tc"
            ...
            """), {
                ...
                'mv': remaining_mv, 'pnl': remaining_pnl, 'pnl_ratio': remaining_ratio,
                'aid': self.account_id, 'tc': ts_code
            })
        ...
        """), {
            'tid': trade_id, 'aid': self.account_id, 'tc': ts_code,
            'price': price, 'qty': quantity, 'amount': trade_amount,
            'comm': commission, 'tax': tax, 'pnl': pnl,
    ...
    def get_positions(self) -> List[Dict[str, Any]]:
        ...
        """), {'aid': self.account_id}).mappings().fetchall()
    ...
    def update_position_prices(self, price_map: Dict[str, float]) -> Dict[str, Any]:
        ...
        for ts_code, new_price in price_map.items():
            ...
            self.db.execute(text("""
                ...
                WHERE account_id = :aid AND ts_code = :tc
            """), {
                'price': new_price, 'mv': new_mv, 'pnl': pnl, 'pnl_ratio': pnl_ratio,
                'aid': self.account_id, 'tc': ts_code
            })
            updated_count += 1
