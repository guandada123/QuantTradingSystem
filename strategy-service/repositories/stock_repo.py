...
def get_stock_pool(db: Session, industry: Optional[str] = None,
    ...
    stocks = (
        query.order_by(StockPool.ts_code)
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def search_stocks(db: Session, keyword: str, limit: int = 20) -> List[Dict]:
    ...
    stocks = (
        ...
        .filter(
            ...
            (StockPool.ts_code.ilike(f'%{keyword}%') |
             StockPool.name.ilike(f'%{keyword}%'))
        )
        .limit(limit)
        .all()
    )
    return [
        {"ts_code": s.ts_code, "name": s.name,
         "industry": s.industry, "market": s.market}
        for s in stocks
    ]
