...
def test_engine_initialization():
    ...
    try:
        ...
        print("✓ StockInsightEngine 初始化成功")
    ...
    try:
        ...
        if rational_results:
            for i, stock in enumerate(rational_results[:3]):
                print(
                    f"  {i + 1}. {stock.get('code', 'N/A')} - {stock.get('selection_type', 'N/A')}"
                )
...
def test_api_structure():
    ...
    try:
        from api.stock_insight import LatestResult, ScanRequest, ScanResult, router
        ...
        # 测试请求模型创建
        request = ScanRequest(scan_type="mainboard", top_n=10, owned_codes=["600519", "000858"])
