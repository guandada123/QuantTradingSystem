...
class FeishuAlertService:
    ...
    async def send_alert(
        ...
        card: dict[str, Any] = {
            ...
            "card": {
                "header": {
                    "title": {"tag": "plain_text", "content": f"🔔 {alert_type.value}: {title}"},
    ...
    async def send_signal_alert(
        self, ts_code: str, action: str, price: float, confidence: float, reason: str
    ):
        """交易信号告警"""
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if result.get("code") == 0:
                        logger.info("[FeishuAlert] 卡片推送成功")
                    else:
                        logger.warning(f"[FeishuAlert] 卡片推送返回异常: {result}")
                else:
                    logger.warning(
                        f"[FeishuAlert] 卡片推送失败 HTTP {resp.status_code}: {resp.text}"
                    )
        except Exception as e:
