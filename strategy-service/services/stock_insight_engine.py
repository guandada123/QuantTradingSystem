...
class StockInsightEngine:
    ...
    def scan_mainboard(self, top_n: int = 10, owned_codes: List[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def scan_ml(self, mode: str = "mainboard", top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(tier1_results) < top_n:
                ...
                tier2_results = self._ml_tier_selection(mode, top_n - len(tier1_results), relaxed=True)
    ...
    def _filter_mainboard_candidates(self, stock_pool: List[Dict], owned_codes: List[str] = None) -> List[Dict]:
        """主板候选股票基础筛选"""
    ...
    def _analyze_candidate(self, candidate: Dict, days: int = 365) -> Optional[Dict]:
        """深度分析候选股票"""
        try:
    ...
    def _select_top_with_sector_diversification(self, all_results: List[Dict], top_n: int) -> List[Dict]:
        """板块去重选择TOP股票"""
