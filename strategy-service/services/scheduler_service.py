...
class TaskSchedulerService:
    ...
    def add_cron_job(self, func, job_id: str, hour: int, minute: int, 
                     args: list = None, kwargs: dict = None,
                     name: str = "", day_of_week: str = None,
                     day: int = None, description: str = "") -> str:
        """添加 cron 定时任务"""
        trigger_kwargs = dict(hour=hour, minute=minute, timezone="Asia/Shanghai")
        if day_of_week:
...
def register_default_tasks(scheduler: TaskSchedulerService):
    ...
    scheduler.add_cron_job(
        _job_daily_data_refresh, "daily_data_refresh",
        hour=15, minute=10, name="日行情刷新",
        description="拉取当日K线、更新数据库"
    )
    scheduler.add_cron_job(
        _job_daily_close_settle, "daily_close_settle",
        hour=15, minute=20, name="收盘归总",
        description="市值快照、收益结算"
    )
    scheduler.add_cron_job(
        _job_ai_review, "daily_ai_review",
        hour=15, minute=30, name="AI每日复盘",
        description="AI分析当日持仓表现"
    )

    # -------- 盘前任务(09:00前) --------
    scheduler.add_cron_job(
        _job_market_scan, "market_scan",
        hour=9, minute=0, name="智能选股扫描",
    ...
    scheduler.add_interval_job(
        _job_market_snapshot, "market_snapshot",
        minutes=30, name="大盘快照",
        description="记录指数行情快照"
    )

    # -------- 系统维护(每小时) --------
    scheduler.add_interval_job(
        _job_health_check, "health_check",
        minutes=60, name="健康检查",
        description="各服务健康检查"
    )
...
async def _job_health_check():
    ...
    try:
        ...
        if up_count < len(services):
            logger.warning(f"[定时任务] ⚠️ 部分服务不可用: {[k for k, v in statuses.items() if v != 'UP']}")
