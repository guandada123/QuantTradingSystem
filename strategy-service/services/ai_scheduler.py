...
class AIModelScheduler:
    ...
    def _select_most_capable_model(
        self, 
        task_type: TaskType, 
        complexity: TaskComplexity
    ) -> str:
        ...
        candidate_models.sort(
            key=lambda m: self.MODEL_CAPABILITY_SCORE.get(m, 0),
            reverse=True
        )
        ...
        for model in candidate_models:
            ...
            if estimated_cost <= self.remaining_budget:
                logger.info(f"选择最强能力模型：{model}（能力评分：{self.MODEL_CAPABILITY_SCORE[model]}）")
                return model
    ...
    def _select_balanced_model(
        ...
        # 检查预算
        if best_model:
            estimated_cost = self._estimate_task_cost(task_type, self.MODEL_COST_COEFFICIENT[best_model])
            if estimated_cost > self.remaining_budget:
