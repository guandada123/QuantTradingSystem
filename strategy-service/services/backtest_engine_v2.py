...
class EnhancedBacktestEngine:
    ...
    def _get_data_service(self):
        ...
        if not hasattr(self, "_data_service_ref") or self._data_service_ref is None:
            try:
                ...
                logger.warning(
                    "BacktestEngine: DataService 初始化失败，降级为无兜底模式",
                )
    ...
    def run(
        ...
        logger.info(
            "BacktestEngine: 回测完成",
            ts_codes=",".join(self.config.ts_codes),
            strategies=",".join(self.config.strategies),
            trade_days=len(common_dates),
            total_trades=result.total_trades,
            total_return=round(result.total_return, 4),
            sharpe=round(result.sharpe_ratio, 4),
            data_prep_ms=round((t2 - t1) * 1000, 1),
            signal_ms=round((t3 - t2) * 1000, 1),
            simulation_ms=round((t5 - t4) * 1000, 1),
            duration_ms=round(duration_ms, 1),
        )
        return result
        ...
        if data is None:
            ...
            for ts_code in self.config.ts_codes:
                ...
                if market_data:
                    ...
                    logger.debug(
                        "BacktestEngine: 获取行情数据",
                        ts_code=ts_code,
                        records=len(market_data),
                    )
            logger.info(
                "BacktestEngine: 行情数据就绪",
                stocks=len(data),
                ts_codes=",".join(data.keys()),
            )

        if benchmark_data is None:
            benchmark_data = self.fetch_benchmark_data(self.config.start_date, self.config.end_date)
            logger.debug(
                "BacktestEngine: 基准数据就绪",
                benchmark=self.config.benchmark,
                records=len(benchmark_data) if benchmark_data else 0,
            )
    ...
    def _build_bt_indices(
        ...
        logger.debug(
            "BacktestEngine: 信号索引就绪",
            stocks=len(data),
            strategies=",".join(self.config.strategies),
            total_dates=len(common_dates),
            total_signals=total_signals,
            avg_signals_per_stock=round(total_signals / max(len(data), 1), 1),
        )
    ...
    def _build_regime_index(
        ...
        if cfg.regime_per_stock or not benchmark_data:
            ...
            logger.debug(
                "BacktestEngine: 市场状态过滤就绪（按个股）",
                mode="per_stock",
                stocks=len(regime_index),
                ma_fast=cfg.regime_ma_fast,
                ma_slow=cfg.regime_ma_slow,
                adx_threshold=cfg.regime_adx_threshold,
            )
            ...
            logger.debug(
                "BacktestEngine: 市场状态过滤就绪（按基准指数）",
                mode="benchmark",
                benchmark=cfg.benchmark,
                days=len(regime_index_all),
                stocks=len(regime_index),
                ma_fast=cfg.regime_ma_fast,
                ma_slow=cfg.regime_ma_slow,
            )
    ...
    def walk_forward(
        ...
        if not data:
            logger.warning(
                "BacktestEngine: Walk-Forward 无数据",
                ts_code=ts_code,
                strategy=strategy,
            )
        ...
        max_windows = (total_len - train_days) // step_days
        logger.info(
            "BacktestEngine: Walk-Forward 开始",
            ts_code=ts_code,
            strategy=strategy,
            total_days=total_len,
            train_days=train_days,
            test_days=test_days,
            step_days=step_days,
            estimated_windows=max(0, max_windows),
        )
        ...
        while offset + train_days + test_days <= total_len:
            ...
            logger.debug(
                "BacktestEngine: Walk-Forward 窗口完成",
        ...
        duration_ms = (time.monotonic() - t0) * 1000
        logger.info(
            "BacktestEngine: Walk-Forward 完成",
            ts_code=ts_code,
            strategy=strategy,
            windows=len(windows),
            overall_test_return=round(overall_test_return, 4),
    ...
    def _grid_search(
        ...
        logger.debug(
            "BacktestEngine: 网格搜索开始",
            ts_code=ts_code,
            strategy=strategy,
            train_days=len(data),
            param_keys=",".join(keys),
            param_combos=total_combos,
        )
