...
class TdxQuoteProvider(QuoteProvider):
    ...
    def get_realtime_quote(self, ts_code: str) -> Dict[str, Any]:
        """获取单只股票实时行情"""
        result = self._call_mcp('get_quotes', {
            'codes': [ts_code],
            'fields': ['code', 'name', 'price', 'open', 'high', 'low',
    ...
    def get_batch_realtime(self, ts_codes: List[str]) -> List[Dict[str, Any]]:
        ...
        return [self._format_quote(c, quote_map.get(self._normalize_code(c), {}))
                for c in ts_codes]
        ...
        return [
            {
                ...
                'pct_change': float(quote_map.get(self._normalize_code(c), {}).get('pct_change', 0)),
                'timestamp': datetime.now().isoformat(),
                'source': 'tdx',
            }
            for c in codes
    ...
    def get_daily_kline(
        ...
        """获取日K线"""
        result = self._call_mcp('get_kline', {
            'code': ts_code,
            'period': 'day',
            'count': limit,
        })
    ...
    def get_fundamental(self, ts_code: str) -> Dict[str, Any]:
        ...
        result = self._call_mcp('get_indicators', {
            'code': ts_code,
...
class AKShareQuoteProvider(QuoteProvider):
    ...
    def _empty_quote(self, ts_code: str) -> Dict[str, Any]:
        return {
            'ts_code': ts_code, 'name': ts_code,
            'price': 0.0, 'pct_change': 0.0, 'volume': 0,
            'timestamp': datetime.now().isoformat(), 'source': 'akshare',
        }

    def _empty_index(self, code: str) -> Dict[str, Any]:
        return {
            'code': code.split('.')[0], 'name': code,
...
class QuoteProviderFactory:
    ...
    def get_provider(self, source: Optional[str] = None) -> QuoteProvider:
        """获取指定或默认的数据提供者"""
...
# 全局工厂实例（延迟初始化）
_factory: Optional[QuoteProviderFactory] = None

def get_quote_provider(source: Optional[str] = None) -> QuoteProvider:
    """获取全局行情提供者"""
    global _factory
    if _factory is None:
